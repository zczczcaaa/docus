...
- **User-agent detection**: `curl` requests are automatically redirected
- **Root path**: `/` → redirects to `/llms.txt`
- **Documentation pages**: Coming soon - `/path` → `/raw/path.md`
...
Both commands will return the `/llms.txt` content instead of the HTML homepage.
